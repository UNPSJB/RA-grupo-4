        ...
        <div style={{ color: "black", display: "flex", flexDirection: "column", gap: "0.75rem" }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: "0.6rem" }}>
                {seccionesFiltradas.map((seccion, index) => {
                     ...
                     const isUltimos = seccionesFiltradas.length % 3 !== 0 && index >= seccionesFiltradas.length - (seccionesFiltradas.length % 3);
                     const flexBasis = isUltimos ? `calc(${100 / (seccionesFiltradas.length % 3)}% - 0.6rem)` : "calc(33.33% - 0.6rem)";

                    return (
                        <div key={seccion.id} style={{ flex: `1 1 ${flexBasis}`, minWidth: "280px", maxWidth: "400px", padding: "0.5rem", border: "1px solid #ccc", borderRadius: "6px", backgroundColor: "#fff" }}>
                            <h3 style={{ fontWeight: "600", fontSize: "1rem", marginBottom: "0.3rem", textAlign: "center" }}>
                                {seccion.sigla + " - " + seccion.nombre}
                            </h3>
                            <ResponsiveContainer width="100%" height={250}>
                                <BarChart data={Object.entries(seccion.porcentajes_opciones).map(([opcion, porcentaje]) => ({ opcion, porcentaje }))} margin={{ top: 20, right: 10, left: 0, bottom: 70 }}>
                                    ...
                                    <Bar dataKey="porcentaje" fill="#4f46e5" barSize={25}>
                                        <LabelList dataKey="porcentaje" position="top" formatter={(value: any) => (typeof value === "number" ? `${value}%` : value)} />
                                    </Bar>
                                </BarChart>
                            </ResponsiveContainer>
                        </div>
            ...
            <div style={{ padding: "0.5rem", border: "1px solid #ccc", borderRadius: "6px", backgroundColor: "#fff" }}>
                ...
                <textarea
                    style={{ 
                        width: "100%", border: "1px solid #ccc", padding: "0.3rem", borderRadius: "4px",
